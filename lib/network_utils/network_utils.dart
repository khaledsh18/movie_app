...
class NetworkUtils{
  final String apiKey = '98e26b7aa7e058d01ef164a88aac3985';
  ...
  getMovies({required String page,int? genreId}) async{
    ...
    try{
      var response = await http.get(Uri.parse('$baseUrl$apiKey$filters'));
  ...
  getCustomMovies(String type) async{
    ...
    if(type == MovieSelection.trending.name){
      ...
      url = 'https://api.themoviedb.org/3/movie/$type?api_key=$apiKey&language=en-US';
    }
